...
class SignalsInterfaceTests(_TestInterfaces):
    ...
    def generic_sub_bcast_test(self, signal_name, trigger, trigger_args=()):
        """Checks that signal_name signal is broadcast when one of the
           trigger is involved."""
        server_notification = threading.Event()
    ...
    def test_subscription_another_client(self):
        """Checks that a subscription is broadcasted to another client
           who did not orignate the event trigger."""
        ...
        self.generic_sub_bcast_test('player.status',
                                    client2.player.set_volume, ('40',))
    ...
    def test_sub_broadcast_audiopls_update(self):
    ...
    def test_sub_broadcast_recpls_listupdate(self):
        ...
        ans = self.deejayd.audiolib.get_dir_content().wait_for_answer()
        dir_obj = self.testdata.get_random_element(ans["directories"])
        djpl.load_folders([dir_obj["id"]]).wait_for_answer()

        test_pl_name = self.testdata.get_random_string()
        test_pl_name2 = self.testdata.get_random_string()
    ...
    def test_sub_broadcast_videopls_update(self):
        ...
        trigger_list = (
            (djvideo.load_folders, ([dir_obj["id"]],)),
            (djvideo.shuffle, ()),
    ...
    def test_sub_broadcast_webradio_listupdate(self):
        ...
        test_wr_name = self.testdata.get_random_string()
        test_wr_urls = ['http://' + self.testdata.get_random_string(50)]
    ...
    def test_sub_broadcast_mediadb_vupdate(self):
        ...
        self.generic_sub_bcast_test('library.vupdate',
