 ...
 * style }</li> <li>.gwt-SliderBar-shell-focused
 * { primary style when focused }</li>
...
implements HasValueChangeHandlers<Double>{
    ...
    private class KeyTimer extends Timer {
         ...
         */
        public void schedule(int delayMillis, boolean shiftRight,
                int multiplier) {
            firstRun = true;
    ...
    /**
     * The timer used to continue to shift the knob if the user holds down
     * a key.
     */
    ...
    /**
     * A bit indicating whether or not we are currently sliding
     * the slider bar due to mouse events.
     */
    ...
    private void startSliding(boolean highlight, boolean fireEvent) {
        if (highlight) {
            ...
            AbstractImagePrototype.create(
                    resources.sliderSliding()).applyTo(knobImage);
        }
    }
